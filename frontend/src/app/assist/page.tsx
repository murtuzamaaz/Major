...
export default function AssistPage() {
  ...
  // ── Helpers ─────────────────────────────────────────────────────────
  const activeRepoId =
    mode === "url" ? repoId : (selectedProject?.repo_id ?? "");
  const isReady =
  ...
  async function handleUploadRepo() {
    ...
    try {
      ...
      // step 2 — index
      setUrlStep("indexing");
      const idxRes = await fetch(`${BASE_URL}/code-assist/index/${id}`, {
        method: "POST",
      });
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="glass p-8 rounded-lg">
            ...
            <p className="text-muted-foreground mb-8">
              Ask questions about any codebase — from a URL or an
                  ...
                  <>
                    {uploadInfo && (
                      <div className="flex gap-3">
                        <InfoChip
                          label="Files indexed"
                          value={uploadInfo.files_indexed}
                        />
                        <InfoChip
                          label="High-risk files"
                          value={uploadInfo.high_risk_files}
                          accent
                        />
                      </div>
                    )}
              ...
              <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-lg bg-green-500/10 border border-green-500/20">
                ...
                <div>
                  <p className="text-sm font-medium text-green-300">
                    Ready — <span className="font-mono">{activeRepoId}</span>
                  </p>
                ...
                <div className="border-t border-border/40 px-6 py-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {askResponse.sources.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs px-2.5 py-1 rounded-md bg-background border border-border text-muted-foreground font-mono"
                      >
                        {s}
                      </span>
                    ))}
...
function StatusBadge({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-4 py-3 rounded-lg border text-sm ${cls}`}
    >
      {icon}
      {text}
    </div>
...
function InfoChip({
  ...
  return (
    <div
      className={`flex-1 text-center px-3 py-2 rounded-lg border text-sm ${
        accent
